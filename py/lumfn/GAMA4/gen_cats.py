...
odir             = '/global/cscratch1/sd/mjwilson/norberg/'

print(version, odir)

gama             = Table.read('/global/cscratch1/sd/mjwilson/norberg/TilingCatv46.fits')

# SURVEY_CLASS >= 3 for union of GAMA II Main Survey and GAMA I Main Survey
gama             = gama[gama['SURVEY_CLASS'] >= 3]
...
# | (gama['NQ2_FLAG'] >= 1) -- up to the user to deem this reliable.
gama             = gama[(gama['NQ'] >= 3)]
...
# https://arxiv.org/pdf/0910.5120.pdf
# Magnitudes are corrected for Milky-Way extinction using the dust maps of Schlegel et al. (1998) except for fibre magnitudes
# Model magnitudes determined from the best fit of an exponential or de Vaucouleurs profile.
# The shape parameters (major-minor axes ratio, position angle, scale radius) are determined from the r- band image, while only the amplitude is fitted in the other bands.

for band in 'UGRIZ':
    bright_merge['{}MAG_DRED_SDSS'.format(band)] = bright_merge['{}_MODEL'.format(band)]

    del bright_merge['{}_MODEL'.format(band)]
...
if todisk:
    ...
    bright_merge.write('/global/cscratch1/sd/mjwilson/norberg/bright_reachable_v{:.1f}.fits'.format(version), format='fits', overwrite=True)


print('Write directory:  ' + '/global/cscratch1/sd/mjwilson/norberg/bright_reachable_v{:.1f}.fits'.format(version))
...
if todisk:
    bright_merge_obs.write('/global/cscratch1/sd/mjwilson/norberg/bright_v{:.1f}.fits'.format(version), format='fits', overwrite=True)

print('Write directory:  ' + '/global/cscratch1/sd/mjwilson/norberg/bright_v{:.1f}.fits'.format(version))
